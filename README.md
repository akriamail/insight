...
*   **核心数据库服务:** PostgreSQL。
*   **Web 管理界面 (Portainer):** 可视化 Docker 管理，积木式服务组合（v1.2+）。
*   **分层系统管理:**
...
### 2. Web 管理界面 (可选，推荐)
安装 Portainer 后，可以通过 Web 界面可视化管理所有服务：
```bash
./manage.sh
# 选择 5 (高级维护) -> 选择 11 (安装 Portainer)
```
访问 `http://你的服务器IP:9000` 即可使用。详细使用指南请查看 [docs/PORTAINER_GUIDE.md](./docs/PORTAINER_GUIDE.md)
...
*   **8) 🌐 Docker 镜像源配置**: 配置国内/国外镜像加速。
*   **9) ⚠️ 全量数据恢复**: 从历史备份包中选择版本进行还原。
*   **10) 🗑️ 重置系统 (危险)**: 彻底清理所有数据。
*   **11) 🎯 安装 Portainer**: 安装 Web 管理界面，支持积木式服务组合。
